...
> 说明：以下清单用于建立严格发布门禁基线。当前为 Task 1 基线记录，均保持未勾选；最终严格 sign-off 仅在后续任务完成并补齐证据后进行。
...
### Baseline command outcomes (Task 1)
...
- 补充说明：待 Task 4 完成后，strict integration gate 将扩展纳入 additional negative-path tests。
