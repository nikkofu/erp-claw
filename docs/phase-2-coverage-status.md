...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| Command Handlers | 创建主数据、创建采购单、提交流程、审批通过/拒绝、收货入库 | P0 | Wave 1 已完成（主数据 / 采购单 / 审批动作） |
| Query Services | 列表、详情、状态视图、库存余额、审批待办 | P0 | Wave 1 部分完成（采购订单详情查询） |
...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| Admin API | 主数据维护、采购单操作、审批动作、库存查询 | P0 | Wave 1 已完成（主数据 / 建单 / 提交 / 审批 / 详情） |
...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| OLTP Schema | 主数据、采购、库存、审批、应付应收基础表 | P0 | Wave 1 迁移已完成（主数据 / 采购 / 审批基础表） |
...
- 已有采购订单状态机与审批请求状态机
- 已有 Admin API：可建档、建单、提交审批、审批通过/拒绝、查询详情
- 已有 Phase 2 Wave 1 迁移：为 PostgreSQL 仓储预留落表结构
