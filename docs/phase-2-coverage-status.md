...
| Receivable and Payable Basics | 应付单、应收单、付款计划、开票基础引用 | P1 | Wave 3 已启动（应付基础记录） | 已支持从已收货采购单生成应付单并提供详情查询，应收与付款计划仍待补齐。 |
...
| Payable Basics Domain | 应付单、应付状态、付款计划引用 | P1 | Wave 3 部分完成（应付单 open 状态 + 查询） |
...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| Admin API | 主数据维护、采购单操作、审批动作、库存查询 | P0 | Wave 3 已扩展到应付单创建与详情查询 |
...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| OLTP Schema | 主数据、采购、库存、审批、应付应收基础表 | P0 | Wave 3 迁移已扩展到 payable_bill |
...
- 应付基础记录已可由“已收货采购单”生成，应收与付款计划仍待补齐
