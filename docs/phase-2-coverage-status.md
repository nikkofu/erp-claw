...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| Admin API | 主数据维护、采购单操作、审批动作、库存查询 | P0 | Wave 3 已扩展到应付/应收基础接口 |
...
| 模块 | 功能清单 | 优先级 | 当前实现情况 |
| --- | --- | --- | --- |
| OLTP Schema | 主数据、采购、库存、审批、应付应收基础表 | P0 | Wave 3 迁移已扩展到 payable_bill / payable_payment_plan / receivable_bill |
...
- 已有 Phase 2 Wave 1-4 迁移：为 PostgreSQL 仓储预留主数据、采购、审批、库存、应付应收与库存预留表结构
...
- `Wave 3-4 持续推进，已打通“收货 -> 库存预留/出库 -> 应付单 -> 付款计划引用”并补齐应收基础对象`
