...
| Inventory | 入库、出库、预留、调拨、库存台账、可用/预留状态 | P0 | Wave 2 持续推进（收货入库 / inbound+outbound ledger / on-hand + reserved + available / reservation） | 已把入库事实、出库事实、预留事实和可用量视图建立起来，调拨仍待补齐。 |
...
| Inventory Domain | 库存流水、库存预留、入出库事务、调拨、台账与可用量计算 | P0 | Wave 2 持续推进（receipt / inbound+outbound ledger / reservation / balance） |
...
- 已有采购订单状态机与审批请求状态机
- 已有 Admin API：可建档、建单、提交审批、审批通过/拒绝、收货入库、库存预留、查询详情和库存余额
...
- 还没有调拨和更完整 movement 维度（当前以 inbound + outbound + reservation 为主）
