...
| Inventory | 入库、出库、预留、调拨、库存台账、可用/预留状态 | P0 | Wave 2 持续推进（收货入库 / inbound ledger / on-hand + reserved + available / reservation） | 已把入库事实、预留事实和可用量视图建立起来，出库与调拨仍待补齐。 |
...
| Inventory Domain | 库存流水、库存预留、入出库事务、调拨、台账与可用量计算 | P0 | Wave 2 持续推进（receipt / inbound ledger / reservation / balance） |
...
- 还没有出库、调拨和更完整 movement 维度（当前以 inbound + reservation 为主）
...
- `Wave 3 持续推进，已打通“收货 -> 应付单 -> 付款计划引用”并补齐应收基础对象`
