...
| Control Plane | Quota / Feature Flag / Model Catalog | 配额、租户特性开关、模型与工具可用性治理 | P2 | 部分实现 | `internal/domain/capability/*`、`internal/application/capability/*`、`internal/infrastructure/persistence/postgres/capability_repository.go`、`internal/bootstrap/capability_catalog.go`、`internal/interfaces/http/router/admin.go`、`migrations/000005_phase1_capability_governance.*`、`migrations/000008_phase1_tool_catalog.*`、`migrations/000010_phase1_agent_capability_policy.*`；当前已经落地 tenant-scoped model catalog、tool catalog、activate/deactivate tenant enablement baseline、最小 agent capability policy binding、独立的 effective capability read surface（只解析显式 `capability-policy` 绑定，把 active entry 放入 effective，把 inactive 或当前无法解析的 entry 放入 stale），以及 opt-in 的 pipeline capability enforcement baseline（显式 payload capability request 才会被阻断）；quota、feature flag、plugin registry 与完整 runtime-side capability enforcement 仍未开始。 |

### 5.3 Execution Plane / Agent 与自动化执行面

| 领域 | 模块 | 功能清单 | 优先级 | 当前实现情况 | 代码锚点 |
| --- | --- | --- | --- | --- | --- |
| Execution Plane | Command Pipeline | 命令进入应用处理器、策略前置、事务边界、审计记录 | P0 | 部分实现 | `internal/application/shared/pipeline.go`、`command.go`、`transaction.go`；当前已经具备 policy decision、transaction boundary、audit record、`REQUIRE_APPROVAL` starter seam，以及 opt-in 的 capability authorizer seam，可在命令 payload 显式声明 `agent_profile_id` / `model_entry_id` / `tool_entry_ids` 时于 handler/approval 之前阻断不被有效允许的 capability 请求；当 capability denial 发生在 policy evaluation 之后时，审计记录保持原 policy decision，并用 `capability_denied` 标记 outcome，但仍未扩展为更完整的 workflow orchestration pipeline |
...
| Execution Plane | Tool Runtime / Query Tool / Command Tool | 工具目录、输入输出 schema、权限、审批、风险控制 | P1 | 部分实现 | 当前已具备 tenant-scoped tool catalog baseline、agent capability policy、effective capability read surface，以及 opt-in 的 pipeline-side capability enforcement baseline；但还没有 tool executor、tool policy binding 的完整 runtime contract、schema contract 和 runtime approval integration |
