...
func TestAdminInventoryLedgerListSupportsSortAndPagination(t *testing.T) {
	gin.SetMode(gin.TestMode)
	container := bootstrap.NewContainer(bootstrap.DefaultConfig())
	h := router.New(router.WithContainer(container))
	...
	postJSONData(t, h, "/api/admin/v1/inventory/outbounds", map[string]any{
		...
		"quantity":       2,
		"reference_type": "shipment",
		"reference_id":   "shp-ledger-query-001",
	})
	postJSONData(t, h, "/api/admin/v1/inventory/outbounds", map[string]any{
		"product_id":     productID,
		"warehouse_id":   warehouseID,
		"quantity":       1,
		"reference_type": "shipment",
		"reference_id":   "shp-ledger-query-002",
	})

	descPage1 := getJSONArrayData(t, h, "/api/admin/v1/inventory/ledger?product_id="+productID+"&warehouse_id="+warehouseID+"&sort=id_desc&page=1&page_size=1")
	if len(descPage1) != 1 {
	...
	ascPage2 := getJSONArrayData(t, h, "/api/admin/v1/inventory/ledger?product_id="+productID+"&warehouse_id="+warehouseID+"&sort=id_asc&page=2&page_size=2")
	if len(ascPage2) != 1 {
...
func TestAdminInventoryLedgerListRejectsInvalidQuery(t *testing.T) {
	gin.SetMode(gin.TestMode)
	container := bootstrap.NewContainer(bootstrap.DefaultConfig())
	h := router.New(router.WithContainer(container))

	cases := []string{
		"/api/admin/v1/inventory/ledger?product_id=prd-001&warehouse_id=wh-001&sort=unknown",
		"/api/admin/v1/inventory/ledger?product_id=prd-001&warehouse_id=wh-001&page=0",
		"/api/admin/v1/inventory/ledger?product_id=prd-001&warehouse_id=wh-001&page_size=0",
	}
	for _, path := range cases {
		env := doJSON(t, h, http.MethodGet, path, nil, http.StatusBadRequest)
		if env.Meta["request_id"] == "" {
